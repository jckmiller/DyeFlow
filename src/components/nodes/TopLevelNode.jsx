...
const TopLevelNode = memo(({ id, data, selected }) => {
  ...
  const borderColor = selected ? '#a5b4fc' : dimmed ? '#444' : accentColor
  ...
  return (
    <div
      ...
      style={{
        ...
        background: dimmed
          ...
          : 'linear-gradient(135deg, #1e1b4b 0%, #2e2b6e 100%)',
    ...
    >
      ...
      <div className="px-6 py-5 flex-1 min-h-0">
        ...
        {data.description && (
          <div className="text-sm text-slate-400 leading-relaxed">{data.description}</div>
        )}
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-500">{childCount} mid-node{childCount !== 1 ? 's' : ''}</span>
          {/* Required badge */}
          <button
      ...
      <Handle type="target" position={Position.Left}  style={{ background: accentColor, border: '2px solid #1e1b4b', width: 13, height: 13 }} />
      <Handle type="source" position={Position.Right} style={{ background: accentColor, border: '2px solid #1e1b4b', width: 13, height: 13 }} />
    </div>
  )
})

TopLevelNode.displayName = 'TopLevelNode'
